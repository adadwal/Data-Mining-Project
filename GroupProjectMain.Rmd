...
ccd$MARRIAGE <- as.numeric(as.character(ccd$MARRIAGE))

credDe <- ccd %>% 
  dplyr::mutate(EDUCATION = ifelse(EDUCATION == 0, 1, EDUCATION)) %>%
  dplyr::mutate(EDUCATION = ifelse(EDUCATION == 5, 4, EDUCATION)) %>%
  dplyr::mutate(EDUCATION = ifelse(EDUCATION == 6, 4, EDUCATION)) %>%
  dplyr::mutate(MARRIAGE = ifelse(MARRIAGE == 0, 3, MARRIAGE))
droplevels.data.frame(credDe)
unique(credDe$EDUCATION)
unique(credDe$MARRIAGE)

# change back to numer
for(i in c(3,7:12)){
  credDe[,i] <- as.numeric(as.character(credDe[,i]))
}
...
```{r}
summary(as.factor(credDe$MARRIAGE))
...
```{r}
glm.fit <- glm(DorND~LIMIT_BAL+SEX+EDUCATION+MARRIAGE+AGE+PAY_0+PAY_2+PAY_3+PAY_4+BILL_AMT1+PAY_AMT1+PAY_AMT2,data=stdccd[,-1],subset = train, family = binomial)
summary(glm.fit)
glm.prob <- predict(glm.fit,TTstdTest,type="response")
...
testY <- ResponseTest
set.seed(3099)
KnnError <- data.frame(k=1:100,TestError=rep(0,100))
# k from 1 to 100
for(i in 1:30){
...
```{r}
library(glmnet)
x <- model.matrix(DorND~.,data=stdccd[,-1])
y <- stdccd$DorND
